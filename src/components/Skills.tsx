...
export function Skills({ isDarkMode }: SkillsProps) {
  ...
  const getColorClasses = (color: string, isDarkMode: boolean) => {
    const colors: { [key: string]: { light: string; dark: string; border: string } } = {
      green: {
  ...
  return (
    <section id="skills" className={`min-h-screen flex items-center justify-center p-8 py-20 transition-colors duration-300 ${
      ...
      <div className="max-w-6xl w-full">
          ...
          {skillCategories.map((category) => {
            ...
            return (
              <div
                ...
                } ${getColorClasses(category.color, isDarkMode).split(' ').find(c => c.startsWith('hover:'))}`}
              >
                {/* Module Header */}
                <div className="flex items-center gap-3 mb-4 pb-3 border-b-2 border-dashed"
                  style={{ borderColor: isDarkMode ? '#334155' : '#e2e8f0' }}
                >
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors duration-300 border-2 ${
                    getColorClasses(category.color, isDarkMode)
                  }`}>
                ...
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill) => (
                    <span
                      key={skill}
                      className={`px-3 py-1 rounded-lg text-sm border transition-colors duration-300 ${
                        isDarkMode 
        ...
        <div className={`mt-12 p-8 rounded-2xl border-2 text-center transition-colors duration-300 ${
          isDarkMode 
            ? 'bg-slate-900 border-slate-800' 
