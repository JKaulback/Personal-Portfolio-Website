...
export function Skills({ isDarkMode }: SkillsProps) {
  ...
  const getColorClasses = (color: string, isDarkMode: boolean) => {
    const colors: { [key: string]: { light: string; dark: string; hoverBorder: string } } = {
      green: {
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {skillCategories.map((category) => {
            ...
            return (
              <div
                key={category.title}
                className={`p-6 rounded-2xl border-2 transition-all duration-300 hover:scale-105 ${isDarkMode
              ...
              >
                ...
                >
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors duration-300 border-2 ${colors.base}`}>
                ...
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill) => (
                    <span
                      key={skill}
                      className={`px-3 py-1 rounded-lg text-sm border transition-colors duration-300 ${isDarkMode
