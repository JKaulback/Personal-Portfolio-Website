...
function Resume({ isDarkMode }) {
    ...
    return (<section id="resume" className="min-h-screen flex items-center justify-center p-8 transition-colors duration-300">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className={`p-4 rounded-xl border-2 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
              ...
              <div className="space-y-6">
                {experience.map((job, index) => (<div key={index} className={`p-4 rounded-xl border-2 ${isDarkMode ? 'bg-yellow-900/20 border-yellow-800/50' : 'bg-yellow-50 border-yellow-200'}`}>
                    ...
                    <ul className={`space-y-2 text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-700'}`}>
                      {job.highlights.map((highlight, i) => (<li key={i} className="flex gap-2">
          ...
          <div className="space-y-6">
            ...
            <div className={`p-4 rounded-xl border-2 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
              <h3 className={`mb-4 pb-2 border-b-2 border-dashed flex items-center gap-2 ${isDarkMode ? 'text-white border-slate-700' : 'text-slate-900 border-slate-300'}`}>
                <lucide_react_1.GraduationCap className="w-5 h-5"/>
                Education
              </h3>
              
              {education.map((edu, index) => (<div key={index} className={`p-4 rounded-xl border-2 ${isDarkMode ? 'bg-yellow-900/20 border-yellow-800/50' : 'bg-yellow-50 border-yellow-200'}`}>
            ...
            <div className={`p-4 rounded-xl border-2 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
              ...
              <div className="space-y-2">
                {certifications.map((cert, index) => (<div key={index} className={`p-3 rounded-lg border-2 text-sm ${isDarkMode
